        ...
        private void OnManageSceneMultiviewClicked()
        {
            try
            {
                ...
                if (response is SuccessResponse success)
                {
                    ...
                    {
                        string folderSpec = com.tgs.mcpforunity.editor.Helpers.ScreenshotPreferences.Resolve(null);
                        string folder;
                        try
                        {
                            folder = com.tgs.mcpforunity.Helpers.ScreenshotUtility.ResolveFolderAbsolute(folderSpec);
                        }
                        ...
                        string projectRelative = normalizedFile.StartsWith(normalizedRoot, System.StringComparison.OrdinalIgnoreCase)
                            ...
                            : normalizedFile;
                        if (com.tgs.mcpforunity.Helpers.ScreenshotUtility.IsUnderAssets(projectRelative))
